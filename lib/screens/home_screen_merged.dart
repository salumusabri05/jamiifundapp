...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchUnreadCounts() async {
    try {
      ...
      // Get unread notification count from NotificationService
      final notificationCount = await NotificationService.getUnreadNotificationCount(userId);
      
      // Get unread chat count from ChatService
      final chatCount = await ChatService.getUnreadMessagesCount(userId);
      
      if (mounted) {
        setState(() {
          _unreadNotificationsCount = notificationCount;
...
class _HomeContentState extends State<HomeContent> {
  ...
  Future<void> _fetchCampaigns() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _errorMessage = 'Failed to load campaigns: ${e.toString().replaceAll('Exception: ', '')}';
          _isLoading = false;
        });
                ...
                ? Center(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.cloud_off, size: 64, color: Color(0xFF8A2BE2)),
                          const SizedBox(height: 16),
                          Text(
                            _errorMessage,
                            textAlign: TextAlign.center,
